...
#define HTTP_CONFIGURE_ENDPOINT "__configure__"
#define HTTP_STATS_ENDPOINT "__stats__"
#define HTTP_INDEX_ENDPOINT "__index__"
#define HTTP_HEALTH_ENDPOINT "__health__"
#define HTTP_UPLOAD_ENDPOINT "__upload__"
#define HTTP_SETKEY_ENDPOINT "__setkey__"

#define HTTP_MAX_KEYLEN 2048
#define HTTP_MAX_VALUELEN 2048
...
is_admin_command(char *key, char **extra)
{
    ...
    if (strcmp(buf, HTTP_STATS_ENDPOINT) == 0 ||
        strcmp(buf, HTTP_INDEX_ENDPOINT) == 0 ||
        strcmp(buf, HTTP_HEALTH_ENDPOINT) == 0 ||
        strcmp(buf, HTTP_CONFIGURE_ENDPOINT) == 0 ||
        strcmp(buf, HTTP_UPLOAD_ENDPOINT) == 0 ||
        strcmp(buf, HTTP_SETKEY_ENDPOINT) == 0)
    {
        key[i++] = 0;
        if (extra)
            *extra = key + i;
...
shardcached_build_index_response(fbuf_t *buf, int do_html, shardcache_t *cache, const char *basepath)
{
    ...
    if (do_html) {
        fbuf_printf(buf,
                    "<html><body>"
                    "<form action=\"__set__\" method=\"POST\" enctype=\"multipart/form-data\"> \
                                 ...
                                 int is_head)
{
    ...
    if (strcmp(key, HTTP_CONFIGURE_ENDPOINT) == 0) {
    ...
    } else if (strcmp(key, HTTP_STATS_ENDPOINT) == 0) {
    ...
    } else if (strcmp(key, HTTP_INDEX_ENDPOINT) == 0) {
        fbuf_t buf = FBUF_STATIC_INITIALIZER;
        ...
        fbuf_destroy(&buf);
    } else if (strcmp(key, HTTP_HEALTH_ENDPOINT) == 0) {
        conn->status_code = 200;
        char *resp = do_html ? "<html><body>OK</body></html>" : "OK";
        mg_printf(conn, HTTP_HEADERS,
                        do_html ? "text/html" : "text/plain",
        ...
        if (!is_head)
            mg_printf(conn, "%s", resp);
    } else if (strcmp(key, HTTP_UPLOAD_ENDPOINT) == 0) {
        fbuf_t htmlBuf = FBUF_STATIC_INITIALIZER;

        fbuf_printf(&htmlBuf,
            "<form action=\"__file__\" method=\"POST\" enctype=\"multipart/form-data\"> \
    ...
    } else if (strcmp(key, HTTP_SETKEY_ENDPOINT) == 0) { 
        fbuf_t htmlBuf = FBUF_STATIC_INITIALIZER;
...
static int
shardcached_post_extract_data(struct mg_connection *conn, char **key, size_t *keyLen, char **value, size_t *valueLen, char **error)
{
    static __thread char local_error[1024];
    char *el = strstr(conn->content, "\r\n");
    if (!el) {
        snprintf(local_error, sizeof(local_error), "Bad Request");
        *error = local_error;
        return -1;
    }
    int ofx = el - conn->content;
    fbuf_t boundary = FBUF_STATIC_INITIALIZER;
    fbuf_add_binary(&boundary, "\r\n", 2);
    fbuf_add_binary(&boundary, conn->content, ofx);
    char var_name[HTTP_MAX_KEYLEN];
    int len = mg_parse_header(conn->content+ofx, "name", var_name, sizeof(var_name));
    if (len != 3 || strncmp(var_name, "key", len) != 0) {
        snprintf(local_error, sizeof(local_error), "Malformed Request");
        *error = local_error;
        fbuf_destroy(&boundary);
        return -1;
    }
    el = strstr(conn->content+ofx, "\r\n\r\n");
    ofx = el - conn->content + 4;
   
    el = strstr(conn->content+ofx, fbuf_data(&boundary));
    if (!el) {
        snprintf(local_error, sizeof(local_error), "Malformed Request");
        *error = local_error;
        fbuf_destroy(&boundary);
    ...
    static __thread char keybuf[HTTP_MAX_KEYLEN];
    static __thread size_t keysize;
    keysize = el - (conn->content + ofx);
    if (keysize == 0) {
        snprintf(local_error, sizeof(local_error), "No Key Specified");
        *error = local_error;
        fbuf_destroy(&boundary);
        return -1;
    } else if (keysize > sizeof(keybuf)) {
        snprintf(local_error, sizeof(local_error), "Key too big");
        *error = local_error;
        fbuf_destroy(&boundary);
        return -1;
    }
    memcpy(keybuf, conn->content + ofx, keysize);
    keybuf[keysize] = 0;
    ofx = el - conn->content + fbuf_used(&boundary)+ 2;

    len = mg_parse_header(conn->content+ofx, "name", var_name, sizeof(var_name));
    if (len != 5 || strncmp(var_name, "value", len) != 0) {
        snprintf(local_error, sizeof(local_error), "Malformed Request");
        *error = local_error;
        fbuf_destroy(&boundary);
        return -1;
    ...
    static __thread char valuebuf[HTTP_MAX_VALUELEN];
    static __thread size_t valuesize;
    valuesize  = el - (conn->content + ofx);
    if (valuesize == 0) {
        snprintf(local_error, sizeof(local_error), "No Value Specified");
        *error = local_error;
        fbuf_destroy(&boundary);
        return -1;
    }
    memcpy(valuebuf, conn->content + ofx, valuesize);
    valuebuf[valuesize] = 0;

    // we are done with the boundary
    fbuf_destroy(&boundary);
    *key = keybuf;
    *value = valuebuf;
    *keyLen = keysize;
...
shardcached_handle_post_request(http_worker_t *wrk, struct mg_connection *conn, char *key)
{
    ...
    if (cp != NULL) {
        ...
        if (strstr(conn->uri, "__set__") != NULL) {
            // skip the boundary;
            ...
            int rc = shardcached_post_extract_data(conn, &key, &keysize, &value, &valuesize, &error);
            if (rc == -1) {
                mg_printf(conn, "HTTP/1.0 400 Bad Request\r\nContent-Length: %d\r\n\r\n%s", error ? strlen(error) : 11, error ? error : "Bad Request");
                conn->status_code = 400;
                return;
            }

            shardcache_set(wrk->cache, key, strlen(key), value, strlen(value), 0, 0, 0, (void *)NULL, (void *)NULL);
            conn->status_code = 200;
...
static int
shardcached_http_recv_handler(struct mg_connection *conn)
{
    int ofx = 0;
    connection_param_t *cp = (connection_param_t *)conn->connection_param;
...
static int
shardcached_http_request_handler(struct mg_connection *conn)
{
...
static int
shardcached_event_handler(struct mg_connection *conn, enum mg_event event)
{
    connection_param_t *cp = conn->connection_param;
    switch(event)
    {
        ...
        case MG_RECV:
            ...
            {
                return shardcached_http_recv_handler(conn);
            }
            return 0;
        case MG_POLL:
        {
        ...
        case MG_REQUEST:
        {
            return shardcached_http_request_handler(conn);
        }
        default:
            break;
    }
                 ...
                 int num_workers)
{
    ...
    for (i = 0; i < num_workers; i++) {
        ...
        wrk->server = mg_create_server(wrk, shardcached_event_handler);
        if (!wrk->server) {
